...
>("journals/deleteJournal", async (journalId, { rejectWithValue }) => {
  try {
    await apiService.delete(`/topic/article/${journalId}`);
    return journalId;
  } catch (err: any) {
    return rejectWithValue(
    ...
    builder
      ...
      .addCase(deleteJournal.fulfilled, (state, action) => {
        ...
        state.journals = state.journals.filter(
          (journal) => journal.articleId !== action.payload
        );
...
export const { setCurrentPage, setFilters, resetFilters } = typeSlice.actions;
